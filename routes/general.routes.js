...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            const { driverCreated, createdDriver } = await Driver.createDriver(
                newDriver
            );
            if (driverCreated) {
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            const { idemployees, firstName, secondName, email, phone, free } =
                req.body;
            ...
            const { driverInfoIsSame, updatedDriver } =
                await Driver.updateDriver(driver);
            if (!driverInfoIsSame && typeof updatedDriver === "object") {
                ...
                return res
                    ...
                    .json({
                        ...
                        message:
                            "Driver was not updated, because the driver info is the same"
                    });
            }
        } catch (error) {
            console.log(error);
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            const { typeOfVehiclesId, identifier, storage_size, free } =
                req.body;
            ...
            const { vehicleCreated, createdVehicle } =
                await Vehicle.createVehicle(newVehicle);
            if (vehicleCreated) {
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            console.log(vehicle);
            const { vehicleInfoIsSame, updatedVehicle } =
            ...
            if (!vehicleInfoIsSame && typeof updatedVehicle === "object") {
                ...
                return res
                    ...
                    .json({
                        ...
                        message:
                            "Vehicle was not updated, because the vehicle info is the same"
                    });
            }
        } catch (error) {
            console.log(error);
...
router.post("/createDeliveryTransaction", async (req, res) => {
    try {
        ...
        const response = await executeTransaction(
            [
                "INSERT INTO packages(user_iduser,weight,height,width,depth,fragile,electronics,oddsized,receiver_iduser) " +
...
router.post("/transaction", async (req, res) => {
    try {
        ...
        const response = await executeTransaction(
            [
                "INSERT INTO user (type_of_user,firstname,secondname,companyname,email,phone,address,duns,zip_city_zipcode_idzipcode,zip_city_city_idcity,password)" +
                    "VALUES(?,?,?,?,?,?,?,?,?,?,?)",
                "INSERT INTO packages(user_iduser,weight,height,width,depth,fragile,electronics,oddsized,receiver_iduser) " +
                    "VALUES (LAST_INSERT_ID(),?,?,?,?,?,?,?,?);"
            ],
