...
router.post(
    ...
    [
        ...
        check("weight")
            .exists({ checkFalsy: true })
            .withMessage("Weight not provided")
            .trim()
            .toFloat()
            .isFloat({ min: 0.0 })
            .withMessage("Wrong value provided"),
        check("height")
            .exists({ checkFalsy: true })
            .withMessage("Height not provided")
            .trim()
            .toFloat()
            .isFloat({ min: 0.0 })
            .withMessage("Wrong value provided"),
        check("width")
            .exists({ checkFalsy: true })
            .withMessage("Width not provided")
            .trim()
            .toFloat()
            .isFloat({ min: 0.0 })
            .withMessage("Wrong value provided"),
        check("depth")
            .exists({ checkFalsy: true })
            .withMessage("Depth not provided")
            .trim()
            .toFloat()
            .isFloat({ min: 0.0 })
            .withMessage("Wrong value provided"),
        check("fragile")
            .exists({ checkFalsy: true })
            .withMessage("Information if packge is fragile not provided")
            .trim()
            .toInt()
            .isInt({ min: 0, max: 1 })
            .withMessage("Wrong value provided"),
        check("electronics")
            .exists({ checkFalsy: true })
            .withMessage("Information if has electronics not provided")
            .trim()
            .toInt()
            .isInt({ min: 0, max: 1 })
            .withMessage("Wrong value provided"),
        check("oddsized")
            .exists({ checkFalsy: true })
            .withMessage("Information if package is odd sized not provided")
            .trim()
            .toInt()
            .isInt({ min: 0, max: 1 })
            .withMessage("Wrong value provided"),
        check("receiver_iduser")
            .exists({ checkFalsy: true })
            .withMessage("Receiver information wasn't provided")
            .trim()
            .toInt()
            .isInt({ min: 0 })
            .withMessage("Wrong value provided")
    ...
    async (req, res) => {
        try {
            ...
            const { packageCreated, createdPackage } =
                await Package.createPackage(newPackage);
            if (packageCreated) {
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            console.log("newPackage inside /addPackage", newPackage.toString());
            const { packageInfoIsSame, updatedPackage } =
                await Package.createPackage(newPackage);
            if (!packageInfoIsSame && typeof updatedPackage === "object") {
                ...
                return res
                    ...
                    .json({
                        ...
                        message:
                            "Package was not updated, because the package info is the same"
                    });
            }
        } catch (error) {
            console.log(error);
