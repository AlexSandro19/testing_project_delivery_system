...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            const { locationCreated, createdLocation } =
                await Location.createLocation(newLocation);
            if (locationCreated) {
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            const { idlocation, typeOfLocationId, address, zipCode, cityId } =
                req.body;
            ...
            const { locationInfoIsSame, updatedLocation } =
                await Location.updateLocation(location);
            if (!locationInfoIsSame && typeof updatedLocation === "object") {
                ...
                return res
                    .status(400)
                    .json({
                        response: updatedLocation,
                        message:
                            "Location was not updated, because the location info is the same"
                    });
            }
        } catch (error) {
            console.log(error);
