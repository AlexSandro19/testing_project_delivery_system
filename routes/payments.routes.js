...
router.post(
    "/addPayment",
    [
        check("typeofpayment_idtypeofpayment")
            .exists({ checkFalsy: true })
            .withMessage("Payment type not provided")
            .trim()
            .toInt()
            .isInt({ min: 0 })
        ...
        check("payed")
            .exists({ checkFalsy: true })
            .withMessage("Information if payment is payed wasn't provided")
            .trim()
            .toInt()
            .isInt({ min: 0, max: 1 })
            .withMessage("Wrong value provided"),
        check("prepaid")
            .exists({ checkFalsy: true })
            .withMessage("Information if payment is prepaid wasn't provided")
            .trim()
            .toInt()
            .isInt({ min: 0, max: 1 })
            .withMessage("Wrong value provided"),
        check("billing_address")
            .exists({ checkFalsy: true })
            .withMessage("Billing address wasn't provided")
            .trim()
            .toInt()
            .isInt({ min: 0 })
            .withMessage("Wrong value provided")
    ...
    async (req, res) => {
        try {
            ...
            const { paymentCreated, createdPayment } =
...
router.post(
    ...
    [
        ...
        check("transactionid")
            .exists({ checkFalsy: true })
            .withMessage("Transaction id not provided")
            .trim()
            .isLength({ max: 20 })
            .withMessage(
                "Transaction id can't be no more than 20 characters long"
            ),
        check("billing_address")
            .exists({ checkFalsy: true })
            .withMessage("Billing address wasn't provided")
    ...
    async (req, res) => {
        try {
            ...
            console.log(
                "updatePayment inside /updatePayment",
                payment.toString()
            );
            const { paymentInfoIsSame, updatedPayment } =
            ...
            if (!paymentInfoIsSame && typeof updatedPayment === "object") {
                ...
                return res
                    ...
                    .json({
                        ...
                        message:
                            "Payment was not updated, because the delivery info is the same"
                    });
            }
