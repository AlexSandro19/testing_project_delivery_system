...
router.post(
    ...
    [
        check("typeOfUser")
            ...
            .withMessage("Type of user not provided")
            .trim()
            .toInt()
            .isInt({ min: 1, max: 2 })
        ...
        check("firstName")
            ...
            .isLength({ max: 45 })
            .withMessage(
                "First name should be no more than 45 characters long"
            ),
        check("secondName")
            .exists({ checkFalsy: true })
            .withMessage("Second name not provided")
            .trim()
            .isAlpha()
            .withMessage("Second name should contain only characters")
            .isLength({ max: 45 })
            .withMessage(
                "Second name should be no more than 45 characters long"
            ),
        check("companyName")
            .exists()
            .withMessage("Company not provided")
            .trim()
            .isAlphanumeric()
            .withMessage(
                "Company name can have only characters and/or numbers provided"
            )
            .isLength({ max: 45 })
            .withMessage(
                "Company name should be no more than 45 characters long"
            ),
        check("email")
            .exists({ checkFalsy: true })
            ...
            .isLength({ max: 45 })
            .withMessage("Email should be no more than 45 characters long"),
        check("phone")
            .exists({ checkFalsy: true })
            ...
            .isLength({ max: 16 })
            .withMessage("Phone should be no more than 16 characters long"),
        check("address")
            .exists({ checkFalsy: true })
            .withMessage("Address not provided")
            .trim()
            .isLength({ max: 70 })
            .withMessage("Address should be no more than 70 characters long"),
        check("duns")
            .exists({ checkFalsy: true })
            .withMessage("DUNS not provided")
            .trim()
            .isNumeric()
            .withMessage("DUNS should contain only numeric values")
            .isLength({ min: 9, max: 9 })
            .withMessage(
                "DUNS should be 9 characters long (format: XXXXXXXXX)"
            ),
        check("zipcode")
            .exists({ checkFalsy: true })
...
router.post(
    ...
    [
        check("idCustomer")
            .exists({ checkFalsy: true })
            .withMessage("Customer not provided")
            .trim()
            .toInt()
            .isInt({ min: 0 })
            .withMessage("Wrong value provided"),
    ...
    async (req, res) => {
        try {
            ...
            if (!userInfoIsSame && typeof updatedUser === "object") {
                ...
                return res
                    ...
                    .json({
                        ...
                        message:
                            "User was not updated, because the user info is the same"
                    });
            }
        } catch (error) {
            console.log(error);
            return res.status(500).json({
                message: "Invalid data",
                errors: [{ value: error.value, msg: error.message }]
            });
        }
    }
);
    ...
    try {
        ...
        if (!user) {
            return res.status(400).json({
                message: "Invalid authorization data",
                errors: [
                    { value: email, msg: "User not found", param: "email" }
                ]
            });
        }
