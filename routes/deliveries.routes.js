...
router.post(
    "/addDelivery",
    [
        check("packages_idpackages")
            .exists({ checkFalsy: true })
            .withMessage("Package not provided")
            .trim()
            .toInt()
            .isInt({ min: 0 })
            .withMessage("Wrong value provided"),
        check("priority")
            .exists({ checkFalsy: true })
            .withMessage("Information about delivery priority wasn't provided")
            .trim()
            .toInt()
            .isInt({ min: 0, max: 1 })
            .withMessage("Wrong value provided"),
        check("payment_idpayment")
            .exists({ checkFalsy: true })
            .withMessage("Payment not provided")
            .trim()
            .toInt()
            .isInt({ min: 0 })
            .withMessage("Wrong value provided"),
        check("international")
            .exists({ checkFalsy: true })
            .withMessage(
                "Information if delivery is international wasn't provided"
            )
            .trim()
            .toInt()
            .isInt({ min: 0, max: 1 })
        ...
        check("start_location")
            .exists({ checkFalsy: true })
            .withMessage("Start location not provided")
            .trim()
            .toInt()
            .isInt({ min: 0 })
            .withMessage("Wrong value provided"),
        check("end_location")
            .exists({ checkFalsy: true })
            .withMessage("End location not provided")
            .trim()
            .toInt()
            .isInt({ min: 0 })
            .withMessage("Wrong value provided"),
        check("message")
            .optional()
            .exists()
            .withMessage("Message not provided")
            .trim()
            .isLength({ max: 150 })
            .withMessage("Message should be no more than 150 characters long"),
        check("start_date")
            .exists({ checkFalsy: true })
            .withMessage("Start date not provided")
            .trim()
            .toDate()
            .withMessage("Value provided is not a date")
    ],
    async (req, res) => {
        try {
            ...
            const estimated_date = await Delivery.getEstimatedDateFromDB(
                start_location,
                end_location,
                start_date
            );
            ...
            const { deliveryCreated, createdDelivery } =
                await Delivery.createDelivery(newDelivery);
            ...
            return res.status(500).json({
                ...
                errors: [{ value: error, msg: error.message }]
            });
        }
    }
);
...
router.post(
    ...
    [
        check("iddeliveries")
            .exists({ checkFalsy: true })
            .withMessage("Delivery not provided")
            .trim()
            .toInt()
            .isInt({ min: 0 })
            .withMessage("Wrong value provided"),
        ...
        check("start_date")
            ...
            .withMessage("Start date not provided")
            .trim()
            .isDate({ format: "YYYY-MM-DD hh:mm:ss" })
            .withMessage(
                "Date has wrong format (should be 'YYYY-MM-DD hh:mm:ss')"
            )
            .isAfter()
            .withMessage("Date selected can't be earlier than today's date.")
            .toDate()
            .withMessage("Value provided is not a date"),
        check("estimated_date")
            .exists({ checkFalsy: true })
            .withMessage("Estimated date not provided")
            .trim()
            .isDate({ format: "YYYY-MM-DD hh:mm:ss" })
            .withMessage(
                "Date has wrong format (should be 'YYYY-MM-DD hh:mm:ss')"
        ...
        check("end_date")
            .optional()
            .exists({ checkFalsy: true })
            .withMessage("End date not provided")
            .trim()
            .isDate({ format: "YYYY-MM-DD hh:mm:ss" })
            .withMessage(
                "Date has wrong format (should be 'YYYY-MM-DD hh:mm:ss')"
        ...
        check("uid")
            .optional()
            .exists()
            .withMessage("UUID not provided")
            .trim()
            .isLength({ max: 20 })
            .withMessage("Message should be no more than 20 characters long") // MUST BE CHANGED
    ],
    async (req, res) => {
        try {
            const errors = validationResult(req);
            ...
            const { deliveryInfoIsSame, updatedDelivery } =
                await Delivery.updateDelivery(delivery);
            ...
            if (!deliveryInfoIsSame && typeof updatedDelivery === "object") {
                ...
                return res
                    .status(400)
                    .json({
                        response: updatedDelivery,
                        message:
                            "Delivery was not updated, because the delivery info is the same"
                    });
            }
        } catch (error) {
            console.log(error);
