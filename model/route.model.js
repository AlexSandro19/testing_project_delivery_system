...
class Route {
    ...
    getIdVehicle() {
        return this.vehicles_idvehicles;
    }
    setIdVehicle(value) {
        this.vehicles_idvehicles = value;
    }

    getIdEmployees() {
        return this.employees_idemployees;
    }
    setIdEmployees(value) {
        this.employees_idemployees = value;
    }

    getTypeOfRoute() {
        return this.typeofroute_idtypeofroute;
    }
    setTypeOfRoute(value) {
        this.typeofroute_idtypeofroute = value;
    }
    ...
    getDeliveries() {
        return this.deliveries_iddeliveries;
    }
    setDeliveries(value) {
        this.deliveries_iddeliveries = value;
    }
    ...
    getStartDateInSqlFormat() {
        ...
        const month =
            this.start_date.getMonth() + 1 >= 10
                ? `${this.start_date.getMonth() + 1}`
                : `0${this.start_date.getMonth() + 1}`;
        const date =
            this.start_date.getDate() >= 10
                ? `${this.start_date.getDate()}`
                : `0${this.start_date.getDate()}`;
        const hours =
            this.start_date.getHours() >= 10
                ? `${this.start_date.getHours()}`
                : `0${this.start_date.getHours()}`;
        const minutes =
            this.start_date.getMinutes() >= 10
                ? `${this.start_date.getMinutes()}`
                : `0${this.start_date.getMinutes()}`;
        const seconds =
            this.start_date.getSeconds() >= 10
                ? `${this.start_date.getSeconds()}`
                : `0${this.start_date.getSeconds()}`;
        // YYYY-MM-DD HH:MI:SS
        console.log(
            "getStartDateInSqlFormat() > this.start_date: ",
            this.start_date
        );
        console.log(
            "getStartDateInSqlFormat() > this.start_date.getMonth: ",
            this.start_date.getMonth()
        );
        console.log(
            "getStartDateInSqlFormat() > this.start_date.ISO: ",
    ...
    getEndDateInSqlFormat() {
        ...
        const month =
            this.end_date.getMonth() + 1 >= 10
                ? `${this.end_date.getMonth() + 1}`
                : `0${this.end_date.getMonth() + 1}`;
        const date =
            this.end_date.getDate() >= 10
                ? `${this.end_date.getDate()}`
                : `0${this.end_date.getDate()}`;
        const hours =
            this.end_date.getHours() >= 10
                ? `${this.end_date.getHours()}`
                : `0${this.end_date.getHours()}`;
        const minutes =
            this.end_date.getMinutes() >= 10
                ? `${this.end_date.getMinutes()}`
                : `0${this.end_date.getMinutes()}`;
        const seconds =
            this.end_date.getSeconds() >= 10
                ? `${this.end_date.getSeconds()}`
                : `0${this.end_date.getSeconds()}`;
        ...
        console.log(
            "getEndDateInSqlFormat() > this.end_date.getMonth: ",
            this.end_date.getMonth()
        );
        console.log(
            "getEndDateInSqlFormat() > this.end_date.ISO: ",
    ...
    equals(route = new Route()) {
        const isEqual =
            route.getIdRoutes() == this.idroutes &&
    ...
    toString() {
        return (
            `idroutes=${this.idroutes}, vehicles_idvehicles= ${this.vehicles_idvehicles}, ` +
    ...
    */
    ...
    static async getRoute(id = Number) {
        try {
            const response = await execute(
                "SELECT * FROM routes WHERE idroutes=?",
                [`${id}`]
            );
    ...
    static async updateRoute(updatedRoute = Route) {
        try {
            const routeFromDB = await execute(
                "SELECT * FROM routes WHERE idroutes=?",
            ...
            if (!updatedRoute.equals(receivedRoute)) {
                ...
                console.log(
                    "Inside Route Model > updateRoute > response: ",
                    response
                );
    ...
    static async deleteRoute(id = Number) {
        try {
            const getDeletedroutes = await execute(
                "SELECT from routes Where idroutes=",
                [`${id}`]
            );
            const response = await execute(
                "DELETE from routes Where idroutes=",
                [`${id}`]
            );
            return new Route(
     ...
     */
    static async createRoute(newRoute = Route) {
        try {
            const response = await execute(
                "INSERT INTO routes(vehicles_idvehicles,employees_idemployees,typeofroute_idtypeofroute,start_location,end_location,international,deliveries_iddeliveries,route_order,start_date,end_date) " +
                    "VALUES (?,?,?,?,?,?,?,?,?,?);",
                [
