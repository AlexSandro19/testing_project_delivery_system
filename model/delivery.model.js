...
class Delivery {
     ...
     */
    getIdDeliveries() {
        return this.iddeliveries;
    }
    setIdDeliveries(value) {
        this.iddeliveries = value;
    }

    getPackageId() {
        return this.packages_idpackages;
    }
    setPackagesId(value) {
        this.packages_idpackages = value;
    }
    ...
    setEstimatedDate(value) {
        this.estimated_date = value;
    }
    getEstimatedDateInSqlFormat() {
        const year = this.estimated_date.getFullYear();
        const month =
            this.estimated_date.getMonth() + 1 >= 10
                ? `${this.estimated_date.getMonth() + 1}`
                : `0${this.estimated_date.getMonth() + 1}`;
        const date =
            this.estimated_date.getDate() >= 10
                ? `${this.estimated_date.getDate()}`
                : `0${this.estimated_date.getDate()}`;
        const hours =
            this.estimated_date.getHours() >= 10
                ? `${this.estimated_date.getHours()}`
                : `0${this.estimated_date.getHours()}`;
        const minutes =
            this.estimated_date.getMinutes() >= 10
                ? `${this.estimated_date.getMinutes()}`
                : `0${this.estimated_date.getMinutes()}`;
        const seconds =
            this.estimated_date.getSeconds() >= 10
                ? `${this.estimated_date.getSeconds()}`
                : `0${this.estimated_date.getSeconds()}`;
    ...
    equals(delivery = new Delivery()) {
        ...
        const isEqual =
            delivery.getIdDeliveries() == this.iddeliveries &&
            ...
            (delivery.getEndDate() != undefined || delivery.getEndDate() != null
                ? delivery.getEndDate().toString()
                : null) ==
                (this.end_date != undefined || this.end_date != null
    ...
    toString() {
        return (
            `iddeliveries= ${this.iddeliveries}, packages_idpackages= ${this.packages_idpackages}, ` +
    ...
    */
    ...
    static async getDelivery(id = Number) {
        try {
            const response = await execute(
                "SELECT * FROM deliveries WHERE iddeliveries=?",
                [`${id}`]
            );
            return new Delivery(
    ...
    static async getEstimatedDateFromDB(
        start_location,
        end_location,
        start_date
    ) {
        try {
            ...
            const response = await execute(
                "SELECT get_estimated_date(?,?,?) AS estimated_date;",
                [start_location, end_location, dateForSQL]
            );
    ...
    static async updateDelivery(updatedDelivery = Delivery) {
        try {
            const deliveryFromDB = await execute(
                "SELECT * FROM deliveries WHERE uid=?;",
                [`${updatedDelivery.getUID()}`]
            );
            console.log(
                "updateDelivery > deliveryFromDB[0]: ",
                deliveryFromDB[0]
            );
            ...
            console.log(
                "updateDelivery > receivedDelivery: ",
                receivedDelivery
            );
            if (!updatedDelivery.equals(receivedDelivery)) {
                ...
                console.log(
                    "Inside Delivery Model > updateDelivery > response: ",
                    response
                );
                if (response.changedRows > 0) {
                    ...
                    return {
                        deliveryInfoIsSame: false,
                        updatedDelivery: undefined
                    };
                }
            } else {
    ...
    static async deleteDelivery(id = Number) {
        try {
            ...
            const response = await execute(
                "DELETE from deliveries Where iddeliveries=",
                [`${id}`]
            );
            return new Delivery(
     ...
     */
    static async createDelivery(newDelivery = Delivery) {
        try {
            const response = await execute(
