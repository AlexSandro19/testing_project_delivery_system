...
} = require("../utility/utility.generators.js");
...
class Payment {
    ...
    getTpeOfPaymentId() {
        return this.typeofpayment_idtypeofpayment;
    }
    setTpeOfPaymentId(value) {
        this.typeofpayment_idtypeofpayment = value;
    }
    ...
    equals(receivedPayment = new Payment()) {
        const isEqual =
            receivedPayment.getIdPayment() == this.idpayment &&
            receivedPayment.getTpeOfPaymentId() ==
                this.typeofpayment_idtypeofpayment &&
    ...
    toString() {
        return (
            `idpayment= ${this.idpayment}, typeofpayment_idtypeofpayment= ${this.typeofpayment_idtypeofpayment}, ` +
    ...
    */
     ...
     * @returns
     */
    static async updatePayment(updatedPayment = new Payment()) {
        try {
            const paymentFromDB = await execute(
                "SELECT * FROM payment WHERE idpayment=?;",
            ...
            if (!updatedPayment.equals(receivedPayment)) {
                ...
                console.log(
                    "Inside Payment Model > updatePayment > response: ",
                    response
                );
    ...
    static async deletePayment(id = Number) {
        try {
            const getDeletedPayment = await execute(
                "SELECT FROM payment WHERE idpayment=",
                [`${id}`]
            );
            if (getDeletedPayment[0]) {
                ...
                return new Payment(
                    getDeletedPayment[0].idpayment,
                    getDeletedPayment[0].typeofpayment_idtypeofpayment,
                    getDeletedPayment[0].amount,
                    getDeletedPayment[0].payed,
                    getDeletedPayment[0].prepaid,
                    getDeletedPayment[0].transactionid,
                    getDeletedPayment[0].billing_address
                );
            } else {
     ...
     */
    static async createPayment(newPayment = new Payment()) {
        try {
            const response = await execute(
                "INSERT INTO payment(typeofpayment_idtypeofpayment,amount,payed,prepaid,transactionid,billing_address) " +
                    "VALUES (?,?,?,?,?,?);",
                [
