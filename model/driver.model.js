...
class Driver {
    ...
    equals(driver = new Driver()) {
        return (
            driver.getIdEmployees() == this.idemployees &&
    ...
    toString() {
        return (
            `idemployees= ${this.idemployees}, firstname= ${this.firstname}, ` +
    ...
    */
    ...
    static async getDriver(id = Number) {
        try {
            const response = await execute(
                "SELECT * FROM driver WHERE idemployees=?",
                [`${id}`]
            );
            return new Driver(
    ...
    static async updateDriver(updatedDriver = Driver) {
        try {
            const driverFromDB = await execute(
                "SELECT * FROM driver WHERE idemployees=?",
            ...
            if (!updatedDriver.equals(receivedDriver)) {
                ...
                if (response.changedRows > 0) {
                    ...
                    return {
                        driverInfoIsSame: false,
                        updatedDriver: undefined
                    };
                }
            } else {
    ...
    static async deleteDriver(id = Number) {
        try {
            const getDeletedDriver = await execute(
                "SELECT from driver Where idemployees=",
                [`${id}`]
            );
            const response = await execute(
                "DELETE from driver Where idemployees=",
                [`${id}`]
            );
            return new Driver(
     ...
     */
    static async createDriver(newDriver = Driver) {
        try {
            const response = await execute(
                "INSERT INTO driver(firstname,secondname,email,phone,free) " +
                    "VALUES (?,?,?,?,?);",
                [
