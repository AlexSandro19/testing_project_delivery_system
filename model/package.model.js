...
class Package {
    ...
    getReceiverId() {
        return this.receiver_iduser;
    }
    setReceiverId(value) {
        this.receiver_iduser = value;
    }

    equals(receivedPackage = new Package()) {
        const isEqual =
            receivedPackage.getIdPackage() == this.idpackages &&
    ...
    toString() {
        return (
            `idpackages= ${this.idpackages}, user_iduser= ${this.user_iduser}, ` +
    ...
    */
    ...
    static async getPackage(id = Number) {
        try {
            const response = await execute(
                "SELECT * FROM Packages WHERE idpackages=?",
                [`${id}`]
            );
    ...
    static async updatePackage(updatedPackage = Package) {
        try {
            const packageFromDB = await execute(
                "SELECT * FROM packages WHERE idpackages=?;",
            ...
            if (!updatedPackage.equals(receivedPackage)) {
                ...
                if (response.changedRows > 0) {
                    ...
                    return {
                        packageInfoIsSame: false,
                        updatedPackage: undefined
                    };
                }
            } else {
    ...
    static async deletePackage(id = Number) {
        try {
            const getDeletedPackage = await execute(
                "SELECT from Packages Where idpackages=",
                [`${id}`]
            );
            const response = await execute(
                "DELETE from Packages Where idpackages=",
                [`${id}`]
            );
            return new Package(
     ...
     */
    static async createPackage(newPackage = Package) {
        try {
            const response = await execute(
