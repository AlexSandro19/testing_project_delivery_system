 ...
 *                              provided queries.
 */
const executeTransaction = (queries = Array(String), params = Array(Array)) => {
    try {
        if (!pool) {
            throw new Error(
                ...
                connection.beginTransaction((error) => {
                    if (error) {
                        connection.rollback(() => {
                            console.log(error);
                            console.log(
                                "ERROR HAS OCCURED WITHIN THE TRANSACTION"
                            );
                            connection.release();
                            reject(error);
                        });
 ...
 *
 */
const execute = (query = String, params = Array(String) | object) => {
    try {
        if (!pool)
            throw new Error(
                "Pool was not created. Ensure pool is created when running the app."
